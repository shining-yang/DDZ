...
public:
    ...
    //
    // No default constructor supported. Caller should always pass HWND to bind
    // PictureEx with a window on which it'll paint when construct the object.
    // Because this class has been modified to fit my Win32 program needs, and
    // it's no longer derived from CStatic.
    //
    // shining  2009-05
    //
    //CPictureEx();
    CPictureEx(HWND hWnd);
    virtual ~CPictureEx();
    ...
    BOOL Load(HMODULE hModule, LPCTSTR szResourceName,LPCTSTR szResourceType);
    ...
    //
    // 设置窗口背景图（底图），PictureEx将保存位图数据的拷贝。
    //
    // PictureEx默认将窗口背景刷为COLOR-3DFACE，参考函数 PrepareDC()。
    // 对于透明GIF图片，这样处理，并不能获得理想透明效果。因此，特引入此函数。
    //
    // 如果窗口不调用本函数进行背景位图的设置，或调用时传递参数 NULL，则PictureEx
    // 图像背景为COLOR-3DFACE。
    //
    void SetBkBitmap(HBITMAP hBkBitmap);

protected:
    void FillBkBitmap(int x, int y, int cx, int cy);
