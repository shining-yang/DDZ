...
CPictureEx::CPictureEx(HWND hWnd)
{
...
BOOL CPictureEx::Load(HGLOBAL hGlobal, DWORD dwSize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            UNREFERENCED_PARAMETER(nCurFrame);
...
void CPictureEx::UnLoad()
{
    ...
    // Destroy this DC when AnimateThread exit. Since it's always created there.
    //if (m_hDispMemDC)
    //{
    ...
    //
    // If user unloads the picture, it doesn't mean that user will never use this
    // picture object again. We should reserve some properties of the picture 
    // object rather than reset them to default value as constructor does.
    //
...
BOOL CPictureEx::Draw()
{
    ...
    if (IsAnimatedGIF())
    {
        ...
        DWORD nThreadId;
        m_hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)_ThreadAnimation,
            this, 0, &nThreadId);
        if (!m_hThread)
        {
            assert(_T("Draw: Couldn't start a GIF animation thread\n"));
            return FALSE;
        }
        //else
    ...
    else
    {
        if (m_pPicture)
        {
            ...
            {
                FillBkBitmap(0, 0, m_PictureSize.cx, m_PictureSize.cy);
            }
                ...
                0, hmHeight, hmWidth, -hmHeight, NULL) == S_OK)
            {
                ...
                rc.left = 0;
                rc.top  = 0;
                rc.right= m_PictureSize.cx;
                rc.bottom=m_PictureSize.cy;
...
BOOL CPictureEx::Load(LPCTSTR szFileName)
{
    ...
    file = CreateFile(szFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
    if (file == INVALID_HANDLE_VALUE)
    {
        //MessageBox(NULL,_T("PictureEx loads file failed."),_T("ERROR"),MB_OK|MB_ICONERROR);
        return FALSE;
    };
    dwSize = GetFileSize(file,NULL);

    hGlobal = GlobalAlloc(GMEM_MOVEABLE|GHND,dwSize);
    if (!hGlobal)
    {
    ...
    if(ReadFile(file,pData,dwSize,&byte,NULL) == FALSE)
    {
        GlobalFree(hGlobal);
        CloseHandle(file);
        return FALSE;
...
BOOL CPictureEx::Load(HMODULE hModule, LPCTSTR szResourceName, LPCTSTR szResourceType)
{
	assert(szResourceName);
	assert(szResourceType);

    //HRSRC hPicture = FindResource(GetModuleHandle(NULL),szResourceName,szResourceType);
    HRSRC hPicture = FindResource(hModule, szResourceName, szResourceType);
    HGLOBAL hResData;

    //if (!hPicture || !(hResData = LoadResource(GetModuleHandle(NULL),hPicture)))
...
UINT WINAPI CPictureEx::_ThreadAnimation(LPVOID pParam)
{
    ...
    // Always CloseHandle after caller wait-on-object.
    //
...
void CPictureEx::ThreadAnimation()
{
    ...
    while (!m_bExitThread)
    {
        if (m_arrFrames[m_nCurrFrame].m_pPicture)
        {
            ...
            {
                RECT rect;
                rect.left = m_arrFrames[m_nCurrFrame].m_frameOffset.cx;
                rect.top  = m_arrFrames[m_nCurrFrame].m_frameOffset.cy;
                rect.right= m_arrFrames[m_nCurrFrame].m_frameSize.cx;
                rect.bottom=m_arrFrames[m_nCurrFrame].m_frameSize.cy;
            ...
            if (m_arrFrames[m_nCurrFrame].m_nDisposal == 2)
            {
                ...
                {
                    FillBkBitmap(m_arrFrames[m_nCurrFrame].m_frameOffset.cx,
                        m_arrFrames[m_nCurrFrame].m_frameOffset.cy,
                        m_arrFrames[m_nCurrFrame].m_frameSize.cx,
                        m_arrFrames[m_nCurrFrame].m_frameSize.cy);
                }
    ...
    if (m_hDispMemDC != NULL) {
        ::SelectObject(m_hDispMemDC, m_hDispOldBM);
        if (m_hDispMemBM != NULL) {
            ::DeleteObject(m_hDispMemBM);
        }
        ::DeleteDC(m_hDispMemDC);
        m_hDispMemDC = NULL;
                                        ...
                                        UINT *pDisposal)
{
    ...
    *pDisposal = 0;
    ...
    if (nBlock == BLOCK_CONTROLEXT)
    {
        ...
        *pDelay = (UINT)-1; // to indicate that there was no delay value
...
void CPictureEx::OnPaint()
{
    ...
    if (nPaintWidth > 0)
    {
        ...
        ::BitBlt(dc, 0, 0, nPaintWidth, nPaintHeight,
            m_hMemDC, m_PaintRect.left, m_PaintRect.top, SRCCOPY);
    }
    else
    {
        ::BitBlt(dc, 0, 0, m_PictureSize.cx, m_PictureSize.cy,
...
void CPictureEx::OnPaint(HDC hdc, LPRECT lpRect)
{
    ::BitBlt(hdc,
        lpRect->left,
        lpRect->top,
...
BOOL CPictureEx::PrepareDC(int nWidth, int nHeight)
{
    ...
    // Fill background color when Draw().
    //
...
void CPictureEx::SetBkColor(COLORREF clr)
{
    ...
    //
    // Set picture background color even if the picture wasn't loaded.
    //
    // And move the fill back color operation to Draw().
    //
    m_clrBackground = clr;
}
...
//
// Set picture background BMP to obtain transparent effect even if it's not loaded.
//
// If an animated GIF paused, call this function will start animation again.
//
void CPictureEx::SetBkBitmap(HBITMAP hBkBitmap)
{
    Stop();

    //
    // 重新设置为第0帧，而不是原来的暂停时的那帧
    //
    m_nCurrFrame = 0;
    ...
    if (hBkBitmap != NULL) {
        m_hBkBitmap = (HBITMAP)CopyImage(hBkBitmap, IMAGE_BITMAP,
...
void CPictureEx::FillBkBitmap(int x, int y, int cx, int cy)
{
    if (m_hBkBitmap != NULL) {
        if (m_hWnd != NULL) {
            ...
            HDC tempDC = CreateCompatibleDC(wndDC);
            assert(tempDC != NULL);

            HBITMAP oldbmp = (HBITMAP)SelectObject(tempDC, m_hBkBitmap);

            BitBlt(m_hMemDC, x, y, cx, cy, tempDC, x, y, SRCCOPY);
